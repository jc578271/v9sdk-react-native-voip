...
- (id)initWithConfig:(NSDictionary *)config {
    ...
    if (self) {
        ...
        // General settings
        {
            ...
            if (![V9SipUtil isEmptyString:self.name]) {
            ...
            cred.realm = ![V9SipUtil isEmptyString:self.regServer] ? pj_str((char *) [self.regServer UTF8String]) : pj_str("*");
            ...
            if (![V9SipUtil isEmptyString:self.contactParams]) {
                cfg.contact_params = pj_str((char *) [self.contactParams UTF8String]);
            }
            if (![V9SipUtil isEmptyString:self.contactUriParams]) {
        ...
        // Registration settings
        {
            ...
            if (![V9SipUtil isEmptyString:self.regContactParams]) {
        ...
        // Transport settings
        {
            if (![V9SipUtil isEmptyString:self.proxy]) {
                cfg.proxy_cnt = 1;
                cfg.proxy[0] = pj_str((char *) [[NSString stringWithFormat:@"%@", self.proxy] UTF8String]);
            }

            cfg.transport_id = [[V9SipEndpoint instance] udpTransportId];
        
            if (![V9SipUtil isEmptyString:self.transport] && ![self.transport isEqualToString:@"UDP"]) {
                if ([self.transport isEqualToString:@"UDP"]) {
                    cfg.transport_id = [[V9SipEndpoint instance] udpTransportId];
                } else if ([self.transport isEqualToString:@"TLS"]) {
                    cfg.transport_id = [[V9SipEndpoint instance] tlsTransportId];
                } else if ([self.transport isEqualToString:@"TCP"]) {
                    cfg.transport_id = [[V9SipEndpoint instance] tcpTransportId];
                } else {
...
- (void)onRegistrationChanged {
    [[[V9SipEndpoint instance].bridge eventDispatcher] sendAppEventWithName:@"RegistrationChanged" body:[self toJsonDictionary]];
}
...
- (NSDictionary *)toJsonDictionary {
    ...
    NSDictionary * registration = @{
        @"status": [V9SipUtil toString:(pj_str_t *) pjsip_get_status_text(info.status)],
        @"statusText": [V9SipUtil toString:&info.status_text],
        @"active": @"test",
        @"reason": @"test"
    };
    ...
    return @{
        @"id": @(self.id),
        @"uri": [V9SipUtil toString:&info.acc_uri],
        @"name": self.name,
